...
pro art_make_aster_v7, key=key, target=target, pexp=pexp, beta=beta, exposures_min=exposures_min, offset=offset, date=date, $
                       ...
                       cat_ra_obj=cat_ra_obj, cat_dec_obj=cat_dec_obj, object=object, start_with_slew=start_with_slew
...
;;  printf,reg,'fk5;circle(',new_ra,',',new_dec,',',art_pix/2,') # text={'+String(pexp,format='(i2)')+'}'
  ...
  for k=0L, n_elements(alpha)-1 do begin
     ...
     for i=0L,n_elements(offset)-1 do begin
        if(n_elements(start_with_slew) gt 0) then begin
        ...
        shift=observation(date=date, texp=exposures_min[i], obsid=obsid, shift=shift, table=table, start_with_slew=start_with_slew, $
                          ...
                          cat_ra_obj=new_ra(i), cat_dec_obj=new_dec(i), object=object)       
