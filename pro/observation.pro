function observation,date=date, texp=texp, ra=ra, dec=dec, obsid=obsid, $
                     ...
                     roll_angle=roll_angle, sun_x0z_angle=sun_x0z_angle, start_with_slew=start_with_slew, $
  ...
  if(n_elements(cat_ra_obj) eq 0) then cat_ra_obj=0.0d0  
  if(n_elements(cat_dec_obj) eq 0) then cat_dec_obj=0.0d0  
  ...
  if(start_with_slew eq 1) then begin
