...
; wide      : wide=1 makes long scans up to 50'
; 
;
;OUTPUT:
; 
...
pro art_make_aster, ra, dec, pa, key=key, pexp=pexp, beta=beta, wide=wide, exposures_min=exposures_min
  ...
  if(wide eq 0) then offset = [1.0, 3.0, 5.0, 7.0, 9.0, 12.0, 15.0, 18.0] else $
     offset = [1.0, 3.0, 5.0, 7.0,  9.0, 12.0, 15.0, 18.0, $
               20.0, 22.0, 24.0, 27.0, 30.0, 40.0, 50.0]
  ...
  for k=0L, n_elements(alpha)-1 do begin
     ...
     for i=0L,n_elements(offset)-1 do begin
        printf,dat,new_ra(i),new_dec(i), pa, exposures_min[i], format='(2f12.6,2f10.2)'
